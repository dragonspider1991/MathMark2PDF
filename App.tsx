...
import { ViewMode, DocumentState, InsertType, Preset, PageSize, Template, FontSize } from './types';
...
function App() {
  ...
  const handleScroll = (source: 'editor' | 'preview') => {
    // Only normal sync scroll if Sync Click Mode is NOT active (optional choice, but usually sync scroll and sync click can coexist or be separate)
    // The requirement says "automatically scrolls", but the trigger is "click".
    // We'll keep standard scroll sync for general browsing, unless it interferes.
    // The standard scroll sync here is percent-based which is often inaccurate.
    
    // For now, leaving standard scroll sync as is, as it provides basic continuity.
...
<body>
  ...
  const handleExportPDF = () => {
    ...
    const opt = {
        ...
        jsPDF:        { unit: 'mm', format: doc.pageSize.toLowerCase() as any, orientation: 'portrait' },
            ...
            <div className="flex flex-1 overflow-hidden relative">
                ...
                >
                    ...
                    <textarea
                        ...
                        className={`w-full h-full p-8 font-mono leading-relaxed resize-none outline-none text-gray-800 placeholder-gray-300 ${fontSizeMap[doc.fontSize]} ${isSyncActive ? 'cursor-pointer' : 'cursor-text'}`}
                        ...
                        <button
                            ...
                            className={`pointer-events-auto p-2 rounded-full shadow-lg border transition-all duration-200 group ${
                                isSyncActive 
                                    ...
                                    : 'bg-white text-gray-400 border-gray-200 hover:text-brand-red hover:border-brand-red/50'
                            }`}
                ...
                >
                    <div 
                    id="markdown-preview" 
                    className={`prose prose-red max-w-none p-8 md:p-12 markdown-body template-${doc.template}`}
                    style={{
